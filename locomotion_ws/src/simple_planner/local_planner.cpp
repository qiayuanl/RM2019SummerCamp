...
class LocalPlanner{
 ...
 private:
  ...
  void FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    double yaw = GetYawFromOrientation(robot_pose.pose.orientation);

    double diff_x = traj.poses[0].pose.position.x
        - robot_pose.pose.position.x;
    double diff_y = traj.poses[0].pose.position.y
        - robot_pose.pose.position.y;

    double vx_map = 0.5 * diff_x;
    double vy_map = 0.5 * diff_y;

    cmd_vel.linear.x = cos(yaw) * vx_map + sin(yaw) * vy_map;
    cmd_vel.linear.y = -sin(yaw) * vx_map + cos(yaw) * vy_map;
